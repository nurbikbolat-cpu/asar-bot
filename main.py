...
@router.callback_query(F.data.startswith("block_") | (F.data == "safety_rules"))
async def send_block_info(callback: CallbackQuery):
    data = callback.data
    content = ""
    ...
    if data == "block_1":
        content = (
            ...
            "• <b>Проблема:</b> Мелкие покупатели переплачивают за розницу на базарах и «платят за воздух», заказывая полупустые Газели.\n"
            "• <b>Решение:</b> Бот объединяет мелкие заказы соседей в один большой оптовый заказ напрямую с завода.\n"
            "• <b>Логистика:</b> Система находит фуру крупного застройщика, которая едет попутно, и закидывает заказ «прицепом»."
        )
    elif data == "block_2":
        content = (
            "🔍 <b>Блок 2. Прозрачность рынка и Борьба с откатами</b>\n\n"
            "• <b>Проблема:</b> Серые схемы, накрутки цен, некачественный материал и прорабы в доле с хитрыми продавцами.\n"
            "• <b>Решение:</b> Маркетплейс выводит честных поставщиков «на свет». Цены заводов прозрачны.\n"
            "• <b>Честный рейтинг:</b> Оставить отзыв поставщику может только тот, кто реально купил через бот."
        )
    elif data == "block_3":
        content = (
            "🤝 <b>Блок 3. Конвейер взаимопомощи (Точки А, Б, С)</b>\n\n"
            "• <b>Проблема:</b> В одиночку строить дом нереально — 90% сил уходит на пустую беготню между бетономешалкой и стеной.\n"
            "• <b>Решение:</b> Соседи запускают «живой конвейер»:\n"
            "  - <b>Точка А (Раствор):</b> один человек только месит.\n"
            "  - <b>Точка Б (Логистика):</b> второй носит на тачке.\n"
            "  - <b>Точка С (Мастер):</b> хозяин кладет кирпич.\n"
            "• <b>Результат:</b> Работа закрывается за 3–4 часа вместо недели."
        )
    elif data == "block_4":
        content = (
            "🤹 <b>Блок 4. Бартер талантов и Экономика «Баурсаков»</b>\n\n"
            "• <b>Проблема:</b> Услуги профи стоят дорого, а прямым бартером меняться сложно.\n"
            "• <b>Решение:</b> Внедрение внутренней валюты — <b>BaurSack</b>.\n"
            "  - Помог соседу — твой баланс <i>растет</i>.\n"
            "  - Пришли строить твой дом — твой баланс <i>уменьшается</i>."
        )
    elif data == "block_5":
        content = (
            "♻️ <b>Блок 5. Эко-утилизация и Строительный шеринг</b>\n\n"
            "• <b>Проблема:</b> После стройки остался мусор (бой кирпич, доски), выбросить нельзя (штраф), а место занимает.\n"
            "• <b>Решение:</b> Выставляете мусор с пометкой «Отдам даром / Самовывоз». Сосед забирает бой кирпича для засыпки ям, вы получаете чистый участок бесплатно."
        )
    elif data == "safety_rules":
        content = (
            "🛡️ <b>Правила безопасности и Регламент:</b>\n\n"
            "1. <b>Информационный посредник:</b> Администрация проекта Asar предоставляет площадку для связи и не несёт юридической или финансовой ответственности за сделки.\n"
            "2. <b>Материальная ответственность:</b> Участник, берущий во временное пользование чужой инструмент, несет полную ответственность за его сохранность.\n"
